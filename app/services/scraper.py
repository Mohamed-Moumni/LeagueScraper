...
class Scraper:
    ...
    async def generate_cookies(self, domain: Optional[str] = None) -> List:
        """
        ...
        domain = domain or "www.sofascore.com"
        ...
        try:
            ...
            filtered_cookies = [
                cookie for cookie in cookies if cookie.domain == domain
            ]
            
            self.cookies = filtered_cookies
            logger.info(f"Generated {len(filtered_cookies)} cookies for domain {domain}")
    ...
    async def set_cookies(self, cookies: Optional[List] = None):
        ...
        logger.debug(f"Set {len(zd_cookies)} cookies in browser")
    ...
    async def get_json_from_page(self, tab, selector: str = "#__NEXT_DATA__") -> Optional[dict]:
        """
    ...
    async def get_api_response(self, url: str, use_cookies: bool = True) -> Optional[dict]:
        """
        ...
        try:
            async with httpx.AsyncClient(timeout=15.0) as client:
                response = await client.get(url, headers=headers, cookies=cookie_dict)
